...
- **Leju Gym npm catalog mappings (Issue #085).** Added the registry-declared
  ...
  `@leju-gym/gym-mcp`, while excluding the unscoped security placeholder and
  unrelated packages that do not provide those commands.
...
### Changed
- **Native Plugin completion (Issue #062).** Added static completion for Plugin subcommands and positional names/arguments without scanning plugin directories or invoking MDTK, external tools, or the network.
